...
import { InProduct } from './types/product'
import { useProductsStore } from '../products'
import { supabase } from '@/utils/supabaseClient';

export const useCartStore = defineStore('cart', () => {
  const cart = ref<InProduct[]>([])
  ...
  const syncCartWithSupabase = async () => {
    ...
    if (localCart.length > 0) {
      const cartToInsert = localCart.map((item: InProduct) => ({
        user_id: user.id,
        product_id: item.id,
  ...
  const addProduct = async (product: InProduct) => {
    if (!product.id) {
    ...
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) {
      const localCart: InProduct[] = JSON.parse(localStorage.getItem('cart') || '[]');
      const existingItem = localCart.find((p: InProduct) => p.id === product.id);
    ...
    const existingItem = cart.value.find((p: InProduct) => p.id === product.id);
  ...
  const removeProduct = async (productId: string) => {
    ...
    if (user) {
      ...
      const updateCart = localCart.filter((i: InProduct) => i.id !== productId)
